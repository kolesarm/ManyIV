...
particular, letting $M$ denote the annihilaror matrix associated with the matrix
...
to terms of order $O((k+l)/n)^2)$, and should have a negligible effect on the estimates unless the ratio $(k+l)/n$ is quite large. With this approximation, the calculations are quite fast even for large sample sizes:
```{r}
...
the @cd93 test developed in @kolesar17md to make the test robust to many
instruments and many exogenous regressors. The command `IVoverid` takes the
results of the IV regression as an argument.
...
matrix). Let $\eye_{m}$ denote the $m\times m$ identity matrix, and let
${Z}_{\perp}=(\eye_{n}-\Hm{W}){Z}$ denote the residual from the sample
projection of ${Z}$ onto $W$.

Define matrices $S$ and $T$ as in @kolesar17md: \begin{align*}
T&=Y'\Hm{\bz_{\perp}}Y/n,& S&=Y'(\eye_{n}-\Hm{Z,W})Y/(n-k-\ell). \end{align*}
...
described in @kolesar17md. In the remainder of this vignette, we briefly
...
\begin{equation*}
  ...
  =\frac{\sum_{i=1}^{n}\hat{\epsilon}_{i}(\kappa)\hat{R}_{i}^{2}}{n^{2}(T_{22}-m(\kappa)S_{22})^{2}}.
\end{equation*}
...
The Sargan test statistic is given by $nm_{\min}/(1-p/n-\ell/n+m_{\min}))$, and
...
test is described in @kolesar17md [Section 6].
