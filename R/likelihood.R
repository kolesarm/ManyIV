...
li.fit <- function(d) {
    ...
    list(estimate=be, se=se, lam=lam, Om=Om)
}
...
re.fit <- function(d) {
    ...
    H <- bob(be, Om) * (lam+d$k/d$n) /
        (d$n*lam * (Qs*Om[2, 2] - d$T[2, 2] +
                        c.re*Qs / ((1-c.re)*aoa(be, Om))))

    list(estimate=be, se=sqrt(-H), lam=lam, Om=Om)
}
...
il.fit <- function(d) {
    ...
    Qlam <- function(la) d$ei[2]* (d$n-d$l+d$n*la) / (d$nu+d$n*d$ei[2])

    ff <- function(lam) {
        besselI(d$n*sqrt(lam*Qlam(lam)), d$k/2, expon.scaled=TRUE) /
            besselI(d$n*sqrt(lam*Qlam(lam)), d$k/2-1, expon.scaled=TRUE) -
            sqrt(lam/Qlam(lam))
    }
    if (ff(1e-8)<0 || is.nan(ff(1e-8)))
        return(list(estimate=be, se=Inf, lam=1e-6, Om=d$S,
                    lik=-logl(be, 1e-6, d$S)))

    lam <- stats::uniroot(ff, c(1e-8, 3*d$ei[2]+1), tol=1e-10)$root
    Om <- (d$nu*d$S+d$n*d$T - d$n*lam*d$ei[2]/Qlam(lam) *
    ...
    if (sum(is.nan(Hes))>0) {
        ...
        se <- sqrt(solve(Hes)[1, 1])
    }

    list(estimate=be, se=se, lam=lam, Om=Om, lik=-logl(be, lam, Om))
}
