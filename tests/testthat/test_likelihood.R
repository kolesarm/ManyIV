...
test_that("Test Invariant likelihood", {
    ...
    IVregILold <- function(d) {
        ...
        ff <- function(be, t) {
            logl(be, exp(t[1]),
                 cbind(c(exp(t[2]), t[3]),
                       c(t[3], (exp(t[4])+t[3]^2)/exp(t[2]))))
        }
        r1 <- re.fit(d)
        start <- c(log(r1$lam), log(d$S[1, 1]), d$S[1, 2], log(det(d$S)))
        il <- stats::optim(start, function(t) ff(r1$estimate, t))
        se.il <- sqrt(solve(numDeriv::hessian(function(t) ff(t[1], t[2:5]),
                                              c(r1$estimate, il$par)))[1, 1])
        Om <- cbind(c(exp(il$par[2]), il$par[3]),
                    c(il$par[3], (exp(il$par[4])+il$par[3]^2)/exp(il$par[2])))
        list(beta=r1$estimate, se=se.il, lam=exp(il$par[1]), Om=Om,
             lik=-logl(r1$estimate, exp(il$par[1]), Om))
    }

    expect_true(il.fit(r1$IVData)$lik-IVregILold(r1$IVData)$lik>0)
    expect_true(il.fit(r3$IVData)$lik-IVregILold(r3$IVData)$lik>0)
