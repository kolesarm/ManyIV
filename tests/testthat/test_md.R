...
test_that("LIML, EMD, UMD standard errors are correct", {
    ...
    VV <- function(be, Om, lam, what="liml") {
        ...
        Vumd <- Vemd + (ss*gam*bob(be, Om)^2+rr)^2 / (bob(be, Om)^2*ss*Xi22^2)
    ...
    expect_equal(VV(r1$estimate, r1$Om, r1$lam, "liml"),
                 md.fit(d, weight="LIML")$se^2*d$n)
    ## EMD: standard errors are based on RE starting values
    expect_equal(VV(r1$estimate, r1$Om, r1$lam, "emd"),
                 md.fit(d)$se^2*d$n)
    ## UMD, valid and invalid
    umd <- umd.fit(d, invalid=FALSE)
    expect_equal(VV(umd$estimate, umd$Om, umd$lam, "umd"), umd$se^2*d$n)
    expect_equal(VV(umd$estimate, umd$Om, umd$lam, "umdi"),
                 umd.fit(d, invalid=TRUE)$se^2*d$n)
...
test_that("RE weight matrix is optimal under normality", {
    ...
    r1 <- re.fit(d)
    Xi22 <- r1$lam / aoa(r1$estimate, r1$Om)
    Del <- MDDelta(r1$estimate, r1$Om, Xi22, d, Gaussian=TRUE)
    tau <- d$k/d$n * (1-d$l/d$n) / (1-d$k/d$n-d$l/d$n)

    tt <- rnorm(1)
    m <- c(r1$estimate, 1) * Xi22^(1/2)
    mm <- outer(m, m)
    Phi <- kronecker(r1$Om, r1$Om) + tt*kronecker(r1$Om, mm) +
        tt*kronecker(mm, r1$Om)
    G <- -L2 %*%  cbind(Xi22^(1/2)*
                            (kronecker(m, c(1, 0)) + kronecker(c(1, 0), m)),
                        kronecker(m, m)/Xi22)
